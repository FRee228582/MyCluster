...
namespace DBUtility
{
    public class DBManagerPool
    {
        ...
        {
            int dbIndex = 0;
            if (forceIndex>0&&forceIndex<DBManagerList.Count)
            {
                DBManagerList[forceIndex].Call(query, callback);
                m_dbCallCountList[forceIndex]++;
            }
            else
            {
                DBManagerList[0].Call(query, callback);
                m_dbCallCountList[0]++;
            }
        ...
        public int GetDBIndex()
        {
            ...
            return _index % DBManagerList.Count;
        }
